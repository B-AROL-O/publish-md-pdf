...
- Images become plain `<img src="...">` rather than Confluence's
  ...
  the other way, `--format md`/`--format pdf` against a page URL _do_
  download that page's attachments and rewrite `ac:image`/`ac:link` to
  point at them (see the main README's [Images and other
  attachments](../README.md#images-and-other-attachments) section) — but
  pushing a page back up, as this document covers, doesn't re-upload
